...
def test_diff_strategy_brief_deterministic_bytes(tmp_path: Path):
...
def test_diff_strategy_brief_matches_fixture(tmp_path: Path):
    cache_dir = tmp_path / "cache"
...
def test_diff_strategy_brief_hash_mismatch_exit_4(tmp_path: Path):
    paths = _paths()
    tampered_left = tmp_path / "left.json"
    tampered_left.write_text(
        paths["left_strategy"].read_text(encoding="utf-8").replace("AAPL", "AAPX"),
        encoding="utf-8",
    )
    cmd = [
        sys.executable,
    ...
    assert "left strategy hash mismatch" in result.stderr.lower()


def test_diff_strategy_brief_cache_only_miss_exit_4(tmp_path: Path):
    paths = _paths()
    cmd = [
        sys.executable,
        "-m",
...
def test_diff_strategy_brief_asof_mismatch_exit_3(tmp_path: Path):
    paths = _paths()
    tampered_right = tmp_path / "right.json"
    right_payload = json.loads(paths["right_strategy"].read_text(encoding="utf-8"))
    ...
    right_meta = json.loads(paths["right_meta"].read_text(encoding="utf-8"))
    right_meta["normalized_strategy_hash"] = sha256(
        tampered_right.read_bytes()
    ).hexdigest()
    right_meta["inputs_hash"] = strategy_inputs_hash(
        right_payload["as_of"],
        right_meta["inputs"]["brief"]["normalized_brief_hash"],
        right_meta["inputs"]["earnings"]["normalized_jsonl_hash"],
        right_meta["inputs"]["catalysts"]["normalized_jsonl_hash"],
    )
