...
from core.regime.catalysts_adapter import inputs_hash
...
def _run_ingest(tmp_path: Path, cache_dir: Path) -> tuple[Path, Path]:
    ...
    out_path = tmp_path / "catalysts.jsonl"
    meta_path = tmp_path / "catalysts.meta.json"
    paths = _paths()
    cmd = [
        sys.executable,
        "-m",
...
def test_ingest_catalysts_deterministic_bytes(tmp_path: Path):
...
def test_ingest_catalysts_output_matches_fixture(tmp_path: Path):
    cache_dir = tmp_path / "cache"
...
def test_ingest_catalysts_hash_validation_fails_exit_4(tmp_path: Path):
    cache_dir = tmp_path / "cache"
    paths = _paths()
    out_path = tmp_path / "catalysts.jsonl"
    meta_path = tmp_path / "catalysts.meta.json"
...
def test_ingest_catalysts_empty_exit_3(tmp_path: Path):
    cache_dir = tmp_path / "cache"
    paths = _paths()
    ...
    assert "no filings" in result.stderr.lower() or "no catalysts" in result.stderr.lower()
